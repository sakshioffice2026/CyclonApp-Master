...
#
# NOTE: compute_pressure_drop() (sanity_check.py) uses TOTAL pressure at
# the inlet and STATIC-only pressure at the outlet -- confirmed via
# debug_pressure_breakdown.py against a real trained checkpoint that the
# outlet's exit velocity head must be treated as a dissipated loss (not
# retained pressure) to match how Shepherd-Lapple's Nh was empirically
# calibrated. With that convention, ratio should land near 1.0, same as
# any other approximate-vs-empirical comparison.
RATIO_WARN_LOW = 0.5
RATIO_WARN_HIGH = 2.0
