"""
train_stairmand_gp_colab.py
────────────────────────────
Run this top-to-bottom in a Google Colab notebook to train the Stairmand
General Purpose (GP) cyclone field model. No command-line flags — every
setting is a plain variable in the CONFIG section below.

This mirrors train_stairmand_colab.py exactly, swapped to the GP ratio
family: it imports STAIRMAND_GP_RATIOS and train_parametric_field_model /
...
Stairmand HE, Stairmand GP). This script is only a convenience wrapper
around it for Colab.

WHAT YOU NEED IN THE COLAB SESSION (same folder, upload all of them):
    field_train.py             (must already contain STAIRMAND_GP_RATIOS —
    ...
    sanity_check.py
    export_onnx.py
    train_stairmand_gp_colab.py   (this file)
...
HOW TO RUN IN COLAB:
    ...
    6. In a new cell:  %run train_stairmand_gp_colab.py
    ...
    7. Both output files are saved straight to your Drive root as they're
       ...
       download from the Colab file browser:
           cyclone_model_stairmand_gp.pth    (checkpoint — keep for later
                                               resume/retraining)
           cyclone_model_stairmand_gp.onnx   (this is the file to give to
                                               the .NET side)
"""
...
# Family of GP-shaped cyclone sizes/flow rates this ONE network will learn
# to cover. Kept identical to the Lapple/Stairmand-HE parametric windows so
# all three families stay directly comparable in the app. Narrow this if
# your real GP designs only ever fall inside a smaller sub-range.
DIAMETER_MIN_MM = 150.0
DIAMETER_MAX_MM = 750.0
FLOW_MIN_CFM = 300.0
FLOW_MAX_CFM = 13000.0

# Operating fluid conditions assumed during training. Matches the HE/Lapple
# parametric models — out of scope to make this multi-condition on this
# first GP pass.
OPERATING_TEMP_C = 25.0
OPERATING_PRESS_KPA = 101.325
GAS_TYPE = "Air"

# Training run length/architecture — same defaults as the HE Colab script.
EPOCHS = 20000
...
# If you already have a partially-trained GP checkpoint (e.g. from a
...
CHECKPOINT_PATH = "/content/drive/MyDrive/cyclone_model_stairmand_gp.pth"
ONNX_OUTPUT_PATH = "/content/drive/MyDrive/cyclone_model_stairmand_gp.onnx"
...
def main() -> None:
    print(
        f"Training Stairmand GP field model: "
    ...
    print(f"Ratios (STAIRMAND_GP_RATIOS from field_train.py): {STAIRMAND_GP_RATIOS}")

    model, scaler, history = train_parametric_field_model(
        rho_fn=fluid_properties,
        ratios=STAIRMAND_GP_RATIOS,
    ...
    print(
        ...
        f"CyclonePredictionService:OnnxModelPathsByType -> STAIRMAND_GP in "
        f"appsettings.json)"
    )
